...
public class CourseLinkHandler extends ColumnHandler {
	...
	public StringBuffer getHTML(String status, IstarUser user, String taskType, int taskID, int reportID) {
		if (reportID == 30){
			...
			return new StringBuffer("<div class='btn-group'> <button type='button' class='btn-u  btn-u-yellow btn-warning dropdown-toggle' data-toggle='dropdown' aria-expanded='false'> "
					+ "Action <i class='fa fa-angle-down'></i> </button> <ul style='min-width: inherit;' class='dropdown-menu' role='menu'><li><a href='/content/edit_lesson?task_id=" + taskID + "'> "
